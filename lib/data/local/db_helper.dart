...
class DBHelper {
  ...
  ///now constructor is private so no multiple instance cannot be made
  ...
  Future<Database> openDB() async {
    ...
    return await openDatabase(dbPath, onCreate: (db, version) {
      ...
      print(
          'table created ducessfulyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy');
      //we can create many table by doing execute
    }, version: 1);

    ///version can be upgraded is schema(strucutre) of table is changed ..not mandatory
  }
  ...
  Future<bool> addNote({required String mTitle, required String mDesc}) async {
    ...
    int rowsEffected = await db.insert(TABLE_NOTE, {
      //here table id is auto increment
  ...
  Future<List<Map<String, dynamic>>> getAllNotes() async {
    //* (in sql) represents the all column name not row
    var db = await getDB();
    ...
    List<Map<String, dynamic>> mData =
        await db.query(TABLE_NOTE); //it represetnt select*from note
    return mData;
  }

  ///update data
  Future<bool> updateNote(
      {String? title, String? desc, required int sno}) async {
    var db = await getDB();
    if (title != "" && desc != "") {
      int rowEffected = await db.update(
          TABLE_NOTE, {COLUMN_NOTE_TITLE: title, COLUMN_NOTE_DESCRIPTION: desc},
