...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super
        .initState(); //it will run for first time before any func while opening app and then donot run and cannot be async
  ...
  Widget getBottomSheetView(String errorMessage, bool condition, int sno) {
    return Container(
      ...
      child: Column(
        children: [
          Text(condition ? "Add Note" : "Edit Note",
            ...
            children: [
              Expanded(child: OutlinedButton(onPressed: () async {
                ...
                if (myyTitle.isNotEmpty && myyDesc.isNotEmpty) {
                  bool check = condition ? await dbRef!.addNote(
                      mTitle: myyTitle, mDesc: myyDesc) : await dbRef!
                      .updateNote(sno: sno, title: myyTitle, desc: myyDesc);
                  if (check) {
                    getNotes();
                  }
                }
